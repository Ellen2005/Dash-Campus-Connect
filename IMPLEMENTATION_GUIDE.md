...
## Latest setup (June 2026)

1. Configure `.env`: `DATABASE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`
2. `npx prisma migrate deploy` — applies `prisma/migrations/20250604120000_platform_sync`
3. `npx prisma generate`
4. Supabase SQL Editor: run `scripts/supabase-storage-rls.sql`
5. Create public storage buckets: `posts`, `avatars`, `covers`, `stories`, `events`, `marketplace`
6. `npm run dev` (port 9002)

**Student approval flow:** `POST /api/admin/approve-user` updates auth + DB, runs `assignStudentToCommunities()`, sends notification.

**Event modal:** use `dialog-panel` on modals — not `obsidian-card` (hover translate caused shaking).

**Excluded:** password reset, email verification, future calendar/library features.
