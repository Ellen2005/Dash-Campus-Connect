#!/bin/bash
# Setup Dash API routes

cd "$(dirname "$0")"

# Create directories
echo "Creating API route directories..."
mkdir -p src/app/api/admin/fields
mkdir -p src/app/api/admin/levels  
mkdir -p src/app/api/admin/students
mkdir -p src/app/api/communities
mkdir -p src/app/api/auth/registration-fields

# Create fields API
cat > src/app/api/admin/fields/route.ts << 'EOF'
...
echo "✓ Created src/app/api/admin/fields/route.ts"

# Create levels API
cat > src/app/api/admin/levels/route.ts << 'EOF'
import { prisma } from "@/lib/prisma";
import { requireAdminSession } from "@/lib/require-admin";
...
echo "✓ Created src/app/api/admin/levels/route.ts"

# Create students API  
cat > src/app/api/admin/students/route.ts << 'EOF'
import { prisma } from "@/lib/prisma";
import { requireAdminSession } from "@/lib/require-admin";
import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
...
echo "✓ Created src/app/api/admin/students/route.ts"

# Create communities API
cat > src/app/api/communities/route.ts << 'EOF'
import { prisma } from "@/lib/prisma";
import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
...
echo "✓ Created src/app/api/communities/route.ts"

# Create registration fields API
cat > src/app/api/auth/registration-fields/route.ts << 'EOF'
import { prisma } from "@/lib/prisma";
import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
...
echo "✓ Created src/app/api/auth/registration-fields/route.ts"

echo ""
echo "✅ All API route files created successfully!"
echo ""
echo "Created files:"
echo "  - src/app/api/admin/fields/route.ts (Fields CRUD management)"
echo "  - src/app/api/admin/levels/route.ts (Levels CRUD management)"
echo "  - src/app/api/admin/students/route.ts (Student filtering)"
echo "  - src/app/api/communities/route.ts (Community operations)"
echo "  - src/app/api/auth/registration-fields/route.ts (Registration fields)"
