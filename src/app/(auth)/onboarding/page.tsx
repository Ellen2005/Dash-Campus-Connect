...
export default function OnboardingPage() {
  ...
  const { dashUser, user } = useAuth();
  const [step, setStep] = useState(1);
  const [isLoading, setIsLoading] = useState(false);
  const [avatar, setAvatar] = useState<string | null>(dashUser?.avatar || null);
  const [bio, setBio] = useState("");
  const [studentId, setStudentId] = useState(dashUser?.studentId || "");
  ...
  const initials = (dashUser?.fullName || "S").split(" ").map((p: string) => p[0]).join("").slice(0, 2).toUpperCase();
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="flex flex-col items-center gap-5">
                ...
                <div className="text-center space-y-1">
                  <h3 className="font-semibold">{dashUser?.fullName || "Student"}</h3>
                  <p className="text-xs text-muted-foreground px-4">@{dashUser?.username || "username"}</p>
                </div>
              </div>
