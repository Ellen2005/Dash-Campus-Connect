...
export async function POST(req: NextRequest, { params }: { params: Promise<{ communityId: string }> }) {
  ...
  const parsed = z.object({ content: z.string().min(1).max(5000), authorId: z.string().min(1) }).safeParse(body);
  ...
  const post = await prisma.communityPost.create({
    data: { content: parsed.data.content, communityId, authorId: parsed.data.authorId },
  });
