...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (role) {
      dataToUpdate.isStudentAdmin = role === "student_admin";
    }
    if (status) {
      if (status === "suspended") dataToUpdate.approvalStatus = "SUSPENDED";
      else if (status === "active") dataToUpdate.approvalStatus = "APPROVED";
      else if (status === "pending") dataToUpdate.approvalStatus = "PENDING";
    }
    ...
    console.error("[update-user] Failed to sync to Prisma:", e);
    // Continue anyway as Supabase was updated
  }
