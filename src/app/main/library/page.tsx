...
export default function LibraryPage() {
  ...
  const handleUpload = async () => {
    if (!title.trim() || !dashUser?.id || !dashUser?.schoolId) return;
    setUploading(true);
    try {
  ...
  return (
    <div className="space-y-6 pb-20 page-enter">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-headline font-extrabold tracking-tight flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-primary" /> Community Library
          </h1>
          <p className="text-sm text-muted-foreground">Share and access study resources, courses, and materials</p>
        </div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-10 h-9 text-sm bg-muted/30"
          />
          {search && (
      ...
      <Tabs value={activeType || "all"} onValueChange={v => setActiveType(v === "all" ? "" : v)}>
        <TabsList className="bg-transparent h-auto p-0 gap-2 border-b w-full justify-start rounded-none overflow-x-auto no-scrollbar">
          <TabsTrigger value="all" className="data-[state=active]:bg-primary/10 data-[state=active]:shadow-none data-[state=active]:text-primary rounded-full px-3 py-1.5 text-xs font-medium">
            All
          </TabsTrigger>
        ...
        <TabsContent value={activeType || "all"} className="pt-4">
          {loading ? (
            ...
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-xs mt-1">Be the first to add a resource!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {r.description && (
                          <p className="text-[11px] text-muted-foreground line-clamp-2 mb-1">{r.description}</p>
                        )}
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Resource Type</Label>
                  ...
                  {RESOURCE_TYPES.map(rt => (
                    <SelectItem key={rt} value={rt}>
                      <div className="flex items-center gap-2">
                        {React.createElement(TYPE_ICONS[rt] || BookOpen, { className: "w-3.5 h-3.5" })} {rt}
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Brief description of this resource..." className="min-h-[60px] resize-none text-sm bg-muted/30" maxLength={2000} />
            </div>
            ...
            {(type === "PDF" || type === "VIDEO" || type === "IMAGE") && (
              <div className="space-y-1.5">
                ...
                <input ref={fileRef} type="file" hidden onChange={e => setUploadFile(e.target.files?.[0] || null)} />
              </div>
            )}
            ...
            {type === "TEXT" && (
              <div className="space-y-1.5">
                ...
                <Textarea value={textContent} onChange={e => setTextContent(e.target.value)} placeholder="Paste your text content here..." className="min-h-[150px] resize-none text-sm bg-muted/30" />
              </div>
            )}
            ...
            {type === "COURSE" && (
              <>
                <div className="space-y-1.5">
                  <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Course URL / Material Link</Label>
                  <Input value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="https://..." className="h-9 text-sm bg-muted/30" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Notes / Content</Label>
                  <Textarea value={textContent} onChange={e => setTextContent(e.target.value)} placeholder="Course notes, syllabus, or description..." className="min-h-[80px] resize-none text-sm bg-muted/30" />
                </div>
              </>
            )}
          </div>
