...
type Community = { id: string; name: string; description: string; members: number; type: string };
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        setCommunities(Array.isArray(json?.communities) ? json.communities : []);
      } catch (error: any) {
    ...
    <div className="space-y-5 pb-16 page-enter">
      ...
      <div className="relative">
        ...
        <TabsList className="bg-transparent h-auto p-0 gap-5 border-b w-full justify-start rounded-none">
          {(["people", "groups", "events", "communities"] as const).map(v => (
          ...
          {loading ? <LoadingState /> : groups.length === 0 ? (
            ...
            <div key={g.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/30 transition-colors animate-in fade-in duration-150" style={{ animationDelay: `${i * 30}ms` }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">{g.members} {t("members")} · {g.type}</p>
              </div>
              <Button
                size="sm"
        ...
        <TabsContent value="communities" className="pt-4 space-y-2">
          {loading ? <LoadingState /> : communities.length === 0 ? (
            ...
            <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted/30 transition-colors animate-in fade-in duration-150" style={{ animationDelay: `${i * 30}ms` }}>
              <div className="w-10 h-10 rounded-xl bg-primary/15 border border-primary/20 flex items-center justify-center shrink-0">
                <Users className="w-4 h-4 text-primary" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">{c.name}</p>
                <p className="text-[11px] text-muted-foreground">{c.members} {t("members")} · {c.type}</p>
              </div>
              <Button size="sm" variant="outline" className="h-7 text-[11px] px-2.5 rounded-full shrink-0">
                View
              </Button>
            </div>
          ))}
