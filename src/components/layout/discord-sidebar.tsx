...
import {
  ...
  ShoppingBag, Search, BookOpen, LifeBuoy, Users, Bell, MessageCircle, Library
} from "lucide-react";
...
export function ServerSidebar() {
  ...
  const canAccessAdmin = dashUser?.role === "student_admin" || dashUser?.role === "admin" || dashUser?.isStudentAdmin;
    ...
    <div className="w-[64px] hidden md:flex flex-col items-center py-3 gap-1 sidebar-bg border-r h-full">
      ...
      <div className="flex-1 flex flex-col items-center gap-1 w-full overflow-y-auto no-scrollbar">
        {loading ? (
          ...
          <div className="text-[10px] text-muted-foreground/50 font-bold px-2 text-center uppercase leading-tight mt-2">
            No comms yet
          </div>
        ) : (
          communities.map((c) => {
            const isActive = pathname.includes(`/communities/${c.id}`);
            const label = c.name.slice(0, 2).toUpperCase();
            return (
        ...
        <Link href="/main/groups?create=true" className="group relative flex items-center justify-center w-full mt-2" title="Create community">
